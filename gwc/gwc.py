        ...
        for y, row in enumerate(self.grid[:-1]):
            for x, _ in enumerate(row):
                ...
                for k in range(4):
                    ...
                    if not ((new_state[1] in np.arange(H)) and (new_state[0] in np.arange(W))) or (new_state == [[0, 1], [1, 3]]).all(axis=1).any(): new_state = state
                    elif new_state[1] == H-1: reward_done += [10, 1] # Reached goal
